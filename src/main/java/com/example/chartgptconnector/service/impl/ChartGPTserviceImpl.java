...
public class ChartGPTserviceImpl implements ChartGPTservice {
     ...
     * @return
     */
    public String askAiContext(String prompt,String openId){
        String askAiResponse = new String();
        ...
        messageJson.put("content", prompt);
        JSONArray messageList = new JSONArray();
        String messageListTempStr = stringRedisTemplate.opsForValue().get(openId);
        ...
        try {
            ...
            stringRedisTemplate.opsForValue().set(openId,messageList.toString(),1, TimeUnit.DAYS);
            askAiResponse = gptResponse.getChoices().get(0).getMessage().getContent();
        } catch (Exception exception) {
            log.info("调用openAI接口异常:" + exception.toString());
            askAiResponse = "调用接口发生异常，请联系管理员！！！";
     ...
     */
    @Override
    public void askAiContextStream(String prompt,String openId,OutputStream outputStream){
        log.info("开始执行askAiContextStream方法！！！");
        try {
            String anser = askAiContext(prompt,openId);
            char[] srt = anser.toCharArray();
            log.info("开始流式输出文本！！！");
            for (int i=0;i< srt.length;i++){
                char temp = srt[i];
